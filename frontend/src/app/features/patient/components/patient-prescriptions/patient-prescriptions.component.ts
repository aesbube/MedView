...
export class PatientPrescriptionsComponent implements OnInit {
  columns = [
    {
      columnDef: 'doctor',
      header: 'Doctor',
      cell: (precription: Prescription) =>
        `${precription.doctorNameAndSurname}`,
    },
    {
      columnDef: 'medicine',
      header: 'Medicine',
    ...
    },
    {
      columnDef: 'status',
      header: 'Status',
      cell: (precription: Prescription) => `${precription.status}`, // Display status
    },
    {
      columnDef: 'action',
      header: 'Action',
      cell: (precription: Prescription) =>
        `${precription.id}-${precription.patientId}`,
    },
  ];
  ...
  isActivePrescription(prescription: Prescription): boolean {
    return prescription.status === PrescriptionStatusModel.ACTIVE; // Correct comparison
  }
}
